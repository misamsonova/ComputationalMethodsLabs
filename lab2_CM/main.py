...
print(hh)
def function(z):
    res = math.sin(2*z**2+3*z+10)
    #res = (z ** 2 + 2) + math.sin(math.sqrt(z ** 2 + 2))
    return res


def newton_polinom(mass_x,mass_y,N,o):
    res = mass_y[0]
    for s in range(1,N):
        sigma = 0.0
        for j in range(s+1):
...
for i in range(0,N+1):
    o = mass_x[i]
    mass_zpl1[i] = newton_polinom(mass_x,mass_y,N,o)
...
for i in range(0,M+1):
    o = mass_x2[i]
    mass_zpl2[i] = newton_polinom(mass_x2,mass_y2,M,o)

for i in range(0,M+1):
    d=abs(mass_zpl2[i] - mass_y2[i])
    print(f'{mass_x2[i]:25}',f'{mass_y2[i]:25}',f'{mass_zpl2[i]:25}',f'{d:25}', sep=" "*2)
...
while min(mass_max)>0.01:
   for i in range(1,100):
        ...
        for j in range(0, M2 + 1):
            o = mass_x_2[j]
            mass_zpl_2[j] = newton_polinom(mass_x_2, mass_y_2, M2, o)
